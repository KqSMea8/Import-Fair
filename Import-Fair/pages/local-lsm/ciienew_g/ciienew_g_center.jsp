...
String locatecell = request.getParameter("locatecell");
...
<body  style="width:2400px;height:1200px;">
	<div id="bgDiv" class="bgC" style="width:2400px;height:1200px;padding:40px 0 40px 0;position:relative;">
    	...
    	<iframe frameborder="0" allowtransparency="true"  id="modelFrame" style="width:2400px;height:1200px;visibility:hidden;position:absolute;top:0px;left:0px;" src="">
    	
    	</iframe>
    	<div class="modelframereturn" id="modelframereturn" style="position:absolute;top:40px;right:40px;display:none;">
    	
    	</div>
    	<div class="screentitle_s" style="display:none;width:100%;height:100px;position:absolute;">
			<div class="logociie_s" style="width:928px;margin-left:-464px;"></div>
			<div style="clear:both;"></div>
			<div class="titleTime_s" style="position:relative;">
				<div id="screenTitleTime"  style="text-indent:0px;width: 100%;padding-right:650px;"></div>
...
<script type="text/javascript" src="${ctx}/scripts/local-lsm/ciienew_g/map.js"></script>
...
$(function(){
	if(isScreenMode=="true"){
		...
		if(FROMMODEL=="ciie"){
			new CIIENEW.NavigatorSingle('场景保障-场景GIS');
		}else{
			new CIIENEW.NavigatorSingle('保障概览-GIS概览');
		}
...
//联的传输子网 - 西区中博会
//联的传输子网 - 青浦虹桥205环
function showTopoOfTransmission(id,param){
    if (param != undefined) {
        var src = BASEPATH+'/pages/local-lsm/topo/deviceInfo.jsp?deviceName='+encodeURIComponent(param);
        $("#transmissionEquipmentTitleSpan").text(param);
        $("#transmissionEquipmentIframe").attr('src', src);
    }
    $("#"+id).show();  
}
...
window.clearInterval(overviews1_);
overviews1=setInterval(function() {
	overviews.init();
},300*1000);
overviews1_=setInterval(function() {
	overviews.Schedule();
},60*60*1000);
</script>
</html>