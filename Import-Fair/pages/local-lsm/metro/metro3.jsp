...
<div style="position:absolute;left:750px;top:50px;">
	<span id="subtitleSpan" class="subtitleSpan"></span>
...
<script>
	...
	var defaultHOTSPOT="地铁";
	$(document).ready(function(){
		$("#mainReturnBtn").on("click",returnMainHotspot);
		LSMConsts.allAreas=[defaultHOTSPOT,defaultHOTSPOT,defaultHOTSPOT];
	...
	function NS_SW_chooseLineBySwf(lineName){
		...
		var colorSpan='';
		var key=lineName.replace("地铁","").replace("号线","");
		var color=LSMConsts.metroLineColorMap[key];
		if(color!=null){
			colorSpan='<span style="cursor:pointer;color:#ffffff;background:'+color+'">&nbsp&nbsp'+lineName+'&nbsp&nbsp</span>';
		}else{
			colorSpan=lineName;
		}
		...
		disneyRightNew.hotspot=lineName;
		disneyRightNew.update();
	}
	function returnMainHotspot(){
		$("#subtitleSpan").html("");
		$("#mainReturnBtn").css("display","none");
		disneyRightNew.hotspot=defaultHOTSPOT;
		disneyRightNew.update();
