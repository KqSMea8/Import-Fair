...
function(t, e) {
	...
	L.Draw.Feature = L.Handler.extend({
		...
		initialize: function(t, e) {
			...
			e && e.shapeOptions && (e.shapeOptions = L.Util.extend({},
			this.options.shapeOptions, e.shapeOptions)),
			L.setOptions(this, e)
		},
		enable: function() {
	...
	L.Draw.Polyline = L.Draw.Feature.extend({
		...
		initialize: function(t, e) {
			...
			e && e.drawError && (e.drawError = L.Util.extend({},
			this.options.drawError, e.drawError)),
		...
		_drawGuide: function(t, e) {
			...
			l = s > n ? s - n: r;
			for (this._guidesContainer || (this._guidesContainer = L.DomUtil.create("div", "leaflet-draw-guides", this._overlayPane)); s > l; l += this.options.guidelineDistance) i = l / s,
			o = {
		...
		_updateGuideColor: function(t) {
			if (this._guidesContainer) for (var e = 0,
			i = this._guidesContainer.childNodes.length; i > e; e++) this._guidesContainer.childNodes[e].style.backgroundColor = t
		},
		...
		_getMeasurementString: function() {
			...
			return t = this._measurementRunningTotal + e.distanceTo(i),
	...
	L.Draw.Polygon = L.Draw.Polyline.extend({
		...
		_vertexChanged: function(t, e) {
			var i; ! this.options.allowIntersection && this.options.showArea && (i = this._poly.getLatLngs(), this._area = L.GeometryUtil.geodesicArea(i)),
	...
	L.Draw.SimpleShape = L.Draw.Feature.extend({
		...
		addHooks: function() {
			L.Draw.Feature.prototype.addHooks.call(this),
			this._map && (this._mapDraggable = this._map.dragging.enabled(), this._mapDraggable && this._map.dragging.disable(), this._container.style.cursor = "crosshair", this._tooltip.updateContent({
				text: this._initialLabelText
			}), this._map.on("mousedown", this._onMouseDown, this).on("mousemove", this._onMouseMove, this))
		},
		removeHooks: function() {
			L.Draw.Feature.prototype.removeHooks.call(this),
			this._map && (this._mapDraggable && this._map.dragging.enable(), this._container.style.cursor = "", this._map.off("mousedown", this._onMouseDown, this).off("mousemove", this._onMouseMove, this), L.DomEvent.off(e, "mouseup", this._onMouseUp, this), this._shape && (this._map.removeLayer(this._shape), delete this._shape)),
			this._isDrawing = !1
		},
		_onMouseDown: function(t) {
			this._isDrawing = !0,
	...
	L.Draw.Circle = L.Draw.SimpleShape.extend({
		...
		_onMouseMove: function(t) {
			...
			this._isDrawing && (this._drawShape(i), e = this._shape.getRadius().toFixed(1), this._tooltip.updateContent({
				text: this._endLabelText,
	...
	L.Edit.Poly = L.Handler.extend({
		...
		_initMarkers: function() {
			...
			for (t = 0, i = a.length; i > t; t++) o = this._createMarker(a[t], t),
	...
	L.Polyline.addInitHook(function() {
		this.editing || (L.Edit.Poly && (this.editing = new L.Edit.Poly(this), this.options.editable && this.editing.enable()), this.on("add",
		function() {
	...
	L.Edit.SimpleShape = L.Handler.extend({
		...
		removeHooks: function() {
			if (this._shape._map) {
				...
				for (var t = 0,
				e = this._resizeMarkers.length; e > t; t++) this._unbindMarker(this._resizeMarkers[t]);
	...
	L.Edit.Rectangle = L.Edit.SimpleShape.extend({
		...
		_createResizeMarker: function() {
			...
			this._resizeMarkers = [];
			for (var e = 0,
			i = t.length; i > e; e++) this._resizeMarkers.push(this._createMarker(t[e], this.options.resizeIcon)),
		...
		_move: function(t) {
			for (var e, i = this._shape.getLatLngs()[0], o = this._shape.getBounds(), a = o.getCenter(), s = [], r = 0, n = i.length; n > r; r++) e = [i[r].lat - a.lat, i[r].lng - a.lng],
		...
		_toggleCornerMarkers: function(t) {
			for (var e = 0,
			i = this._resizeMarkers.length; i > e; e++) this._resizeMarkers[e].setOpacity(t)
		},
	...
	L.Edit = L.Edit || {},
	L.Edit.Polygon = L.Edit.SimpleShape.extend({
		_createMoveMarker: function() {
			var t = this._shape,
			e = t.getCenter();
			this._moveMarker = this._createMarker(e, this.options.moveIcon)
		...
		_createResizeMarker: function() {
			...
			for (var e = 0,
			i = t.length; i > e; e++) {
				var makerofResize = this._createMarker(t[e], this.options.resizeIcon);
				makerofResize.on("click",this._onMarkResizeClick,this)
				this._resizeMarkers.push(makerofResize);
				this._resizeMarkers[e]._cornerIndex = e;
			}
		},
		_onMarkResizeClick:function (e) {
			var index = e.latlng;
			var ts = this._getCorners();
			var newTs = [];
			for(var z=0;z<ts.length;z++){
				newTs.push(ts[z]);
				if(ts[z].lat==index.lat&&ts[z].lng==index.lng){
					var nextMark = ts[(z==ts.length-1?0:(z+1))];
					newTs.push({
						lat : (ts[z].lat+nextMark.lat)/2,
						lng : (ts[z].lng+nextMark.lng)/2
					});
				}
			}
			this._shape.setLatLngs(newTs);
			this.updateMarkers();
		},
		_onMarkerDragStart: function(t) {
			L.Edit.SimpleShape.prototype._onMarkerDragStart.call(this, t);
		...
		_onMarkerDragEnd: function(t) {
			var e, i, o = t.target;
			o === this._moveMarker && (e = this._shape, i = e.getCenter(), o.setLatLng(i)),
			this._toggleCornerMarkers(1),
			this._repositionCornerMarkers(),
			L.Edit.SimpleShape.prototype._onMarkerDragEnd.call(this, t)
		},
		_move: function(t) {
			for (var e, i = this._shape.getLatLngs()[0], o = this._shape, a = o.getCenter(), s = [], r = 0, n = i.length; n > r; r++) e = [i[r].lat - a.lat, i[r].lng - a.lng],
			s.push([t.lat + e[0], t.lng + e[1]]);
			this._shape.setLatLngs(s),
			this._repositionCornerMarkers();
		},
		_resize: function(t) {
			var e;
			this._shape.setLatLngs(this.getLatLnges(t, this._oppositeCorner)),
			e = this._shape,
			this._moveMarker.setLatLng(e.getCenter())
		},
		getLatLnges:function (to,from) {
			var zongresi = this._resizeMarkers.map(function (num,index) {
				return num._latlng
			});
			return zongresi;
		},
	...
	L.Polygon.addInitHook(function() {
		L.Edit.Polygon && (this.editing = new L.Edit.Polygon(this), this.options.editable && this.editing.enable())
	}),
	L.Edit = L.Edit || {},
	...
	L.GeometryUtil = L.extend(L.GeometryUtil || {},
	{
		...
		readableArea: function(t, e) {
			var i;
			return e ? i = t >= 1e4 ? (1e-4 * t).toFixed(2) + " ha": t.toFixed(2) + " m&sup2;": (t *= .836127, i = t >= 3097600 ? (t / 3097600).toFixed(2) + " mi&sup2;": t >= 4840 ? (t / 4840).toFixed(2) + " acres": Math.ceil(t) + " yd&sup2;"),
			i
		},
		readableDistance: function(t, e) {
			var i;
			return e ? i = t > 1e3 ? (t / 1e3).toFixed(2) + " km": Math.ceil(t) + " m": (t *= 1.09361, i = t > 1760 ? (t / 1760).toFixed(2) + " miles": Math.ceil(t) + " yd"),
			i
		}
	}),
	...
	L.Control.Draw = L.Control.extend({
		...
		initialize: function(t) {
			...
			var e, i;
			this._toolbars = {},
			L.DrawToolbar && this.options.draw && (i = new L.DrawToolbar(this.options.draw), e = L.stamp(i), this._toolbars[e] = i, this._toolbars[e].on("enable", this._toolbarEnabled, this)),
			L.EditToolbar && this.options.edit && (i = new L.EditToolbar(this.options.edit), e = L.stamp(i), this._toolbars[e] = i, this._toolbars[e].on("enable", this._toolbarEnabled, this))
		},
		onAdd: function(t) {
	...
	L.Toolbar = L.Class.extend({
		...
		removeToolbar: function() {
			...
			this._modes = {};
			for (var e = 0,
			i = this._actionButtons.length; i > e; e++) this._disposeButton(this._actionButtons[e].button, this._actionButtons[e].callback, this);
			this._actionButtons = [],
	...
	L.Tooltip = L.Class.extend({
		...
		updatePosition: function(t) {
			...
			i = this._container;
			return this._container && (i.style.visibility = "inherit", L.DomUtil.setPosition(i, e)),
			this
		},
	...
	L.DrawToolbar = L.Toolbar.extend({
		...
		initialize: function(t) {
			for (var e in this.options) this.options.hasOwnProperty(e) && t[e] && (t[e] = L.extend({},
			this.options[e], t[e]));
	...
	L.EditToolbar = L.Toolbar.extend({
		...
		initialize: function(t) {
			t.edit && ("undefined" == typeof t.edit.selectedPathOptions && (t.edit.selectedPathOptions = this.options.edit.selectedPathOptions), t.edit = L.extend({},
			this.options.edit, t.edit)),
			t.remove && (t.remove = L.extend({},
			this.options.remove, t.remove)),
	...
	L.EditToolbar.Edit = L.Handler.extend({
		...
		enable: function() { ! this._enabled && this._hasAvailableLayers() && (this.fire("enabled", {
				handler: this.type
		...
		removeHooks: function() {
			...
			this._tooltip.dispose(), this._tooltip = null, this._map.off("mousemove", this._onMouseMove, this))
		},
		...
		_toggleMarkerHighlight: function(t) {
			if (t._icon) {
				...
				L.DomUtil.hasClass(e, "leaflet-edit-marker-selected") ? (L.DomUtil.removeClass(e, "leaflet-edit-marker-selected"), this._offsetMarker(e, -4)) : (L.DomUtil.addClass(e, "leaflet-edit-marker-selected"), this._offsetMarker(e, 4)),
				e.style.display = ""
			}
		},
		...
		_enableLayerEdit: function(t) {
			...
			o = i instanceof L.Marker; (!o || i._icon) && (this._backupLayer(i), this._selectedPathOptions && (e = L.Util.extend({},
			this._selectedPathOptions), o ? this._toggleMarkerHighlight(i) : (i.options.previousOptions = L.Util.extend({
				dashArray: null
			},
		...
		_disableLayerEdit: function(t) {
			...
			e instanceof L.Marker ? (e.dragging.disable(), e.off("dragend", this._onMarkerDragEnd, this)) : e.editing.disable()
		},
		_onMarkerDragEnd: function(t) {
			var e = t.target;
			e.edited = !0
