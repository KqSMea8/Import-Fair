    ...
    i.prototype = {
        ...
        renCas: function() {
            var t = this.ele.find(".carousel-inner")
              ...
              , s = i.eq(0).clone()
              , o = this.ele.width()
               , p =this.ele.find(".carousel-inner").height();
            ...
            if($(this.ele.find(".carousel-inner")).data("ecarts")==true){
                for(var q=0;q<t.find(".carousel-item").length;q++){
                	if(q==0||q==t.find(".carousel-item").length-1){
                		$($(t.find(".carousel-item")[q]).context.childNodes[1]).attr("id",$(t.find(".carousel-item")[q]).context.childNodes[1].id+"_auxiliary");	
                	}
                }
            }
        },
        renIns: function() {
           /* for (var t = this.ele.find(".carousel-item").length - 2, i = '<div class="indicators">', n = 0; n < t; n++)
                i += '<span data-index="' + n + '" class="indicators_span""></span>';
            i += "</div>",
            this.ele.append(i).find(".indicators span").eq(this.index).addClass("active")*/
        },
        renBtns: function() {
        ...
        showBtn: function() {
            this.ele.find(".indicators span").removeClass("active").eq(this.index).addClass("active")
        },
        loop: function() {
