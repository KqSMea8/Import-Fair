...
function i_slide(obj,opt){
  ...
  var start;
	...
	var changeCallBack=opts.changeCallBack;
	var imgTouch=opts.imgTouch;
	...
	var rotateDelta=0;//(opts.w2)*(Math.sqrt(2)*0.5)/2000*(2000-opts.w2)/2;
	var left2_left=-rotateDelta;//-(opts.w2)*(Math.sqrt(2)*0.5);//小图left 左侧
	var left2=(opts.conuntW-opts.w2)+rotateDelta;//小图left 旋转45度
	//小图top
...
//默认轮播
   ...
   function pos_dex(N){ 
	...
	//	alert(z);
		...
		for(i=1;i<=half;i++){
			 ...
			 // li.eq(pre).css("z-index",z);
			  li.eq(pre).animate({"left":left2_left,"width":w2,"height":h2,"top":top2},a_speed);
			} 
			//mid
	       if(length%2==0){
  ...
  //鼠标点击
   ...
   obj[0].addEventListener('touchend', function(event) {
	   var cur=parseInt(now_show(li));
	   if(Math.abs(touchStartX-touchEndX)>10){
		   event.preventDefault();
		   if(touchStartX>touchEndX){
			   pos_dex((cur+1)%length);
		   }else{
			   pos_dex((cur-1)%length);
		   }
	   }else{
