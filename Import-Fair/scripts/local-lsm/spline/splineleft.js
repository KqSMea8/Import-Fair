...
GROUPCUSTOMER.Screen.prototype.isM=null;
var _name=null;
...
GROUPCUSTOMER.Screen.prototype.updateCustomers=function(){
	this.cdm=LSMScreen.CacheDataManager.getInstance();
	this.cdm.getSplineCustomers({isM:this.isM,isLocal:"是",customers_name:_name},this.localCustomersHandler.bind(this));
	this.cdm.getSplineCustomers({isM:this.isM,isLocal:"否",customers_name:_name},this.groupCustomersHandler.bind(this));
	$("#zbkh_Modal").modal("hide");
	$("#customers_name").val("");
};
GROUPCUSTOMER.Screen.prototype.click=function(obgect){
	var customers_num=obgect.id;
	if(customers_num=="media_cust_cnt_id"){
		this.isM="是";
		this.updateCustomers();
	}else{
		if(isScreenMode=="true"){
			if(customers_num=="allCustomer_id"){
				this.isM=null;
				this.updateCustomers();
			}
...
GROUPCUSTOMER.Screen.prototype.model=function(){
	...
	/*if($.cookie('targetType')=="大屏版"){
		$("#APN_cust_cnt").live('click',function(){
