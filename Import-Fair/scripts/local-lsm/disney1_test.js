...
/** 从ScreenBase继承*/
DisneyScreen.ScreenController.prototype=Object.create(LSMScreen.ScreenBase.prototype);
DisneyScreen.ScreenController.prototype.constructor=DisneyScreen.ScreenController;

DisneyScreen.ScreenController.prototype.lineWidth=2;//8
DisneyScreen.ScreenController.prototype.baseChartHeight0=380;
DisneyScreen.ScreenController.prototype.baseChartHeight1=80;
DisneyScreen.ScreenController.prototype.showKpiCount=15;
DisneyScreen.ScreenController.prototype.selectedKpi;
DisneyScreen.ScreenController.prototype.dayStreamTrendData;//娄瑶佳当天累计趋势图接口数据缓存
DisneyScreen.ScreenController.prototype.qualityDataNet={};//网络质量面板的缓存数据 网管
DisneyScreen.ScreenController.prototype.qualityDataSig={};//网络质量面板的缓存数据 信令
DisneyScreen.ScreenController.prototype.qualityDataNetDay={};//累计数据 网管
DisneyScreen.ScreenController.prototype.qualityDataSigDay={};//累计数据 信令

DisneyScreen.ScreenController.prototype.timeChooser=null;
DisneyScreen.ScreenController.prototype.lagLogged=false;
DisneyScreen.ScreenController.prototype.chartLoadMask;

DisneyScreen.ScreenController.prototype.initConfigs=function(){
	this.period=2*60*1000;
};
DisneyScreen.ScreenController.prototype.initGridCols=function()
{
...
DisneyScreen.ScreenController.prototype.initData=function()
{
	_kpis = [];
 ...
 * @function 
 */
DisneyScreen.ScreenController.prototype.initComponents=function()
{
...
//	$("#tabNavigator").append(nav_ul);
	 ...
	 * 同比表对象
	 * @private
	 * @type {DisneyScreen.CompareChart} 
	 */
...
DisneyScreen.ScreenController.prototype.lbMenuHandler = function(evt){
...
DisneyScreen.ScreenController.prototype.expandMenuHandler = function(evt){
	var type=evt.type;
...
DisneyScreen.ScreenController.prototype.showRegionSelector = function(){
...
DisneyScreen.ScreenController.prototype.regionClick = function(evt){
...
DisneyScreen.ScreenController.prototype.zoomRegionDetail = function(){
	...
	var win=new LSMScreen.SimpleWindow({
		...
		beforeClose:function(){
			...
			$(".subtitle_txt_right").css("margin","0px 0 0 140px");
...
DisneyScreen.ScreenController.prototype.zoomCompareChart = function(evt){
	var docWidth=$(document).width();
	var docHeight=$(document).height();
...
DisneyScreen.ScreenController.prototype.changeHotspot = function(spot){
	_hotSpot=spot;
	$(".subtitleSpan").text(spot);
	this.getAllTrend(true);
};
DisneyScreen.ScreenController.prototype.changeKpiType = function(evt){
...
DisneyScreen.ScreenController.prototype.getKpiType = function(){
	var type=$(".KpiTypeBtnDiv").find(".customRadioSelected").attr("name");
	return type.replace("指标","");
};
DisneyScreen.ScreenController.prototype.getSelectedKpisByType = function(){
...
DisneyScreen.ScreenController.prototype.changeDetailShow = function(){
...
DisneyScreen.ScreenController.prototype.changePeriod = function(){
...
DisneyScreen.ScreenController.prototype.showKpiChooser = function(){
	...
	var selectedMap={};
	var html='<ul class="kpichooser">';
	
	var ul1='<ul class="kpiGroup">';//用户数 话务量 流量
	var ul2='<ul class="kpiGroup">';//2G
	var ul3='<ul class="kpiGroup">';//3G
	var ul4='<ul class="kpiGroup">';//4G
	var ul5='<ul class="kpiGroup">';//信令统计
...
//			continue;
		...
		var divStr='<div class="indicators KPI KPT_marginR" >'
			...
			+'</div>'
			+'<div class="KPI_down '+addClass+'"><span class="'+source+'"><input type="checkbox" value="'+kpiId+'" '+checked+'>'+kpiName+'</input></span></div>'
			+'</div>';
		html+='<li class="chooserItem">'+divStr+'</li>';
//		html+='<li ><span class="'+source+'"><input type="checkbox" value="'+kpiId+'" '+checked+'>'+kpiName+'</input></span></li>';
	}
...
DisneyScreen.ScreenController.prototype.showDayCompareChart = function(evt){
	var name=$(evt.currentTarget).attr("name");
	switch(name){
		case "今日累计用户数":
			this.selectedKpi={ignoreInConfig:true,kpiFullName:"总用户数", kpiName:"累计用户数",unit:"人", rate:1, operation:"*", fixed:0,source:LSMConsts.kpiSourceStreamDay};
...
DisneyScreen.ScreenController.prototype.chartReady = function()
{
	++_chartReadyCount;
...
DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlow = function()
{
	var dm=LSMScreen.DataManager.getInstance();
	dm.getHotSpotCustomerCountAndFlowCompare([_hotSpot,LSMConsts.hotspots[1],LSMConsts.hotspots[2]], null,null,this.getTimeType(),
			this.getHotSpotCustomerCountAndFlowCompleteHandler.bind(this), this.getHotSpotCustomerCountAndFlowErrorHandler.bind(this));
};

DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlowCompleteHandler = function(result_)
{ 
	...
	if(result_[_hotSpot]["总用户数"]!=0&&!isNaN(result_[_hotSpot]["总用户数"])){
		document.getElementById("labelCustomerCount").innerHTML = "<h1>" + result_[_hotSpot]["总用户数"] + "&nbsp;<span>人</span></h1>";
	}
	if(result_[_hotSpot]["总用户数"]!=0&&!isNaN(result_[_hotSpot]["总流量"])){
...
DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlowCompareTrend = function()
{
	var dm=LSMScreen.DataManager.getInstance();
...
DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlowTrendCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlowErrorHandler = function()
{
	console.log("获取用户数信息失败");
};
DisneyScreen.ScreenController.prototype.getHotSpotCustomerCountAndFlowTrendErrorHandler = function()
{
	console.log("获取累计用户数，流量趋势图失败");
};

DisneyScreen.ScreenController.prototype.getHotSpotsKpis = function()
{
	
	var date0=new Date();
	var dm=LSMScreen.DataManager.getInstance();
	dm.getHotSpotsKpis([_hotSpot,LSMConsts.hotspots[1],LSMConsts.hotspots[2]], null,this.getTimeType(),
			function(result_){
		if(!this.lagLogged){
			var date1=new Date();
...
DisneyScreen.ScreenController.prototype.getHotSpotsKpisCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.updateNetQuality = function()
{
	var result={};
...
DisneyScreen.ScreenController.prototype.getCurrentBoardKpiValue = function(kpiConfig)
{
	var result={};
...
DisneyScreen.ScreenController.prototype.getHotSpotsKpisErrorHandler = function()
{
	console.log("获取指标信息失败");
};
DisneyScreen.ScreenController.prototype.getAllConfigDomain = function()
{
...
//话务量
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlow = function()
{
	var _params = 
	{
		hotspot:_hotSpot,
...
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlowCompleteHandler = function(result_)
{
	var rebuildData={};
...
DisneyScreen.ScreenController.prototype.updateKpiDataCache = function(target,source)
{
...
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlowErrorHandler = function()
{
	console.log("获取话务量信息失败");
};

//全网趋势图链 要请求三个接口
DisneyScreen.ScreenController.prototype.getAllTrend = function()
{
...
//娄瑶佳接口 全网趋势图
DisneyScreen.ScreenController.prototype.streamAllHandler = function(result)
{
...
//黄文接口 全网趋势图
DisneyScreen.ScreenController.prototype.wsAllHandler = function(result)
{
	this.wsAll=result;
...
//话务量趋势
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlowTrend = function()
{
	var _params = 
	{
	    hotspot:_hotSpot,    
...
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlowTrendCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.getHotSpotTrafficFlowTrendErrorHandler = function()
{
	console.log("获取话务量信息失败");
};

DisneyScreen.ScreenController.prototype.setTabSelectedIndex = function(index)
{
	navBar.setSelectedIndex(index);
};

DisneyScreen.ScreenController.prototype.setTabSelectedLabel = function(label)
{
	if(this.alarmList!=null){
		this.alarmList.switchTab(label);
	}
};
DisneyScreen.ScreenController.prototype.getTimeType = function()
{
...
DisneyScreen.ScreenController.prototype.getComparedKpi = function()
{
...
DisneyScreen.ScreenController.prototype.getComparedKpiCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.getComparedKpiCoreCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.getComparedKpiOutsideCompleteHandler = function(result_)
{
...
DisneyScreen.ScreenController.prototype.getComparedKpiErrorHandler = function()
{
	console.log("获取同比数据失败");
};


DisneyScreen.ScreenController.prototype.getHotSpotsKpisTimeComparedErrorHandler = function()
{
	console.log("获取分类同比数据失败");
};
//外部联动调用
DisneyScreen.ScreenController.prototype.showComparedKpi = function(index_)
{
	var kpiList=this.getSelectedKpisByType();
...
//外部联动调用
DisneyScreen.ScreenController.prototype.showComparedKpiFromAll = function(index_)
{
 ...
 * @function 
 */
DisneyScreen.ScreenController.prototype.update=function(showLoadMask)
{
...
DisneyScreen.ScreenController.prototype.loadData=function(showLoadMask)
{
...
DisneyScreen.ScreenController.prototype.loadQualityData=function(showLoadMask)
{
	//头部话务量趋势图
	this.getHotSpotTrafficFlowTrend();
	//网络质量面板指标值
...
DisneyScreen.ScreenController.prototype.getMarkLine = function(kpiId)
{
	var markLine=null;
...
DisneyScreen.ScreenController.prototype.refreshCompareChart = function(kpiConfig,resultList,allResult,targetName)
{
...
//		    	x:40,
	...
	if(source==LSMConsts.kpiSourceStreamDay){
		list=resultList==null?this.dayStreamTrendData:resultList;
...
DisneyScreen.ScreenController.prototype.showCompareTimeChooser = function(evt)
{
 ...
 * }
 */
DisneyScreen.ScreenController.prototype.loadCustomCompareChartData = function(queryConfig)
{
...
DisneyScreen.ScreenController.prototype.loadCompareChartDataByTimeRange = function(queryConfig)
{
...
DisneyScreen.ScreenController.prototype.refreshCustomerChart = function()
{
	var option = {
		    calculable : false,
		    color:['#f4008b'],
...
DisneyScreen.ScreenController.prototype.refreshTrafficFlowChart = function()
{
...
DisneyScreen.ScreenController.prototype.refreshFlowChart = function()
{
	var option = {
		    calculable : false,
		    color:['#f4008b'],
...
DisneyScreen.ScreenController.prototype.setKpiValue = function(value)
{
	$("#kpiValueLabel").text(value);
};

DisneyScreen.ScreenController.prototype.setRegionArea = function(area)
{
...
DisneyScreen.ScreenController.prototype.setSubArea = function(area,parentHot)
{
	if(area=="子热点"){
...
DisneyScreen.ScreenController.prototype.setTitle = function(title)
{
...
DisneyScreen.CompareChart.prototype=Object.create(LSMScreen.DataChartBase.prototype);
...
DisneyScreen.CustomerCompareChart.prototype=Object.create(LSMScreen.DataChartBase.prototype);
...
DisneyScreen.TrafficFlowCompareChart.prototype=Object.create(LSMScreen.DataChartBase.prototype);
...
DisneyScreen.FlowCompareChart.prototype=Object.create(LSMScreen.DataChartBase.prototype);
...
DisneyScreen.RegionDetail.prototype=Object.create(LSMScreen.ComponentBase.prototype);
DisneyScreen.RegionDetail.prototype.constructor=DisneyScreen.RegionDetail;

//区域热点
DisneyScreen.RegionDetail.prototype.kpiConfigMap={};
//区域热点
DisneyScreen.RegionDetail.prototype.hotspot="";
//双击表格选中的热点
DisneyScreen.RegionDetail.prototype.selectedHotspot="";
//双击大标题放大的回调方法
DisneyScreen.RegionDetail.prototype.spotChangeCallBack;
//双击表格钻取指标趋势图的回调方法
DisneyScreen.RegionDetail.prototype.drillCompareCallBack;
//放大按钮的回调方法
DisneyScreen.RegionDetail.prototype.zoomCallBack;

//钻取子热点还是小区
DisneyScreen.RegionDetail.prototype.drillType="";
DisneyScreen.RegionDetail.prototype.useLargerRow=false;
DisneyScreen.RegionDetail.prototype.showAllTable=false;

DisneyScreen.RegionDetail.prototype.DRILL_CELL="CELL";
DisneyScreen.RegionDetail.prototype.DRILL_SUBHOTSPOT="SUBHOTSPOT";

//热点缓存数据
DisneyScreen.RegionDetail.prototype.streamData={};
DisneyScreen.RegionDetail.prototype.wsData={};
//小区缓存数据
DisneyScreen.RegionDetail.prototype.cellList=[];
DisneyScreen.RegionDetail.prototype.streamCellData={};
DisneyScreen.RegionDetail.prototype.wsCellData={};
DisneyScreen.RegionDetail.prototype.cellCntMap={};
DisneyScreen.RegionDetail.prototype.subHotspots=[];

//表格节点 网管
DisneyScreen.RegionDetail.prototype.tableDom;
//表格节点 信令
DisneyScreen.RegionDetail.prototype.tableDom2;

//jqgrid的jquery对象 网管
DisneyScreen.RegionDetail.prototype.grid;
//jqgrid的jquery对象 信令
DisneyScreen.RegionDetail.prototype.grid2;

DisneyScreen.RegionDetail.prototype.gridHeight=165;
DisneyScreen.RegionDetail.prototype.gridWidth=880;
DisneyScreen.RegionDetail.prototype.pageSize=50;
DisneyScreen.RegionDetail.baseKpiColWidth=200;


DisneyScreen.RegionDetail.currentGridData=[];

DisneyScreen.RegionDetail.currentAllCol1=[];
DisneyScreen.RegionDetail.currentAllCol2=[];
DisneyScreen.RegionDetail.currentShowCol1=[];
DisneyScreen.RegionDetail.currentShowCol2=[];

DisneyScreen.RegionDetail.currentAllCol1Origin=[];
DisneyScreen.RegionDetail.currentAllCol2Origin=[];
 ...
 * @function 
 */
DisneyScreen.RegionDetail.prototype.createComponent=function (){
...
DisneyScreen.RegionDetail.prototype.cellTypeChange=function(evt){
	var type=$(evt.currentTarget).attr("name");
...
DisneyScreen.RegionDetail.prototype.thresholdTypeChange=function(evt){
	var type=$(evt.currentTarget).attr("name");
...
DisneyScreen.RegionDetail.prototype.switchTable=function(type){
...
DisneyScreen.RegionDetail.prototype.updateUpSpot=function(_hotspot){
	this.hotspot=_hotspot;
	this.returnUpSpot();
};
DisneyScreen.RegionDetail.prototype.returnUpSpot=function(){
...
DisneyScreen.RegionDetail.prototype.getColList=function(){
	return this.currentShowCol1;
};
DisneyScreen.RegionDetail.prototype.getColList2=function(){
	return this.currentShowCol2;
};

DisneyScreen.RegionDetail.prototype.fillCols=function(source,originCopy){
	...
	for(i=0;i<all.length;i++){
		...
		result.push({
			...
			name : kpiKey,
			index : kpiKey,
			width : DisneyScreen.RegionDetail.baseKpiColWidth,
			unit: record.unit,
			sortable:true
		});
	}
...
DisneyScreen.RegionDetail.prototype.getColConfigId=function(withShowTag){
...
DisneyScreen.RegionDetail.prototype.queryColConfig=function(){
	var dm=LSMScreen.DataManager.getInstance();
...
DisneyScreen.RegionDetail.prototype.updateGridCol=function(){
	this.queryColConfig();
};
DisneyScreen.RegionDetail.prototype.updateShowCol=function(){
...
DisneyScreen.RegionDetail.prototype.pagingData=function(btn){
...
DisneyScreen.RegionDetail.prototype.getSelectedRowData=function(){
...
DisneyScreen.RegionDetail.prototype.isHotspot=function(){
	var ctrlDisplay=$(".regionDetailCtrl").css("display");
	return ctrlDisplay=="none";
};
DisneyScreen.RegionDetail.prototype.getCellType=function(){
//	return $(".cellTypeInput:checked").val();
	var cellType=$(".cellTypeCtrl .customRadioSelected").attr("name");
	return cellType;
};
DisneyScreen.RegionDetail.prototype.getThresholdType=function(){
//	return $(".cellTypeInput:checked").val();
	var thresholdType=$(".cellThresholdCtrl .customRadioSelected").attr("name");
	return thresholdType;
};
DisneyScreen.RegionDetail.prototype.getShowType=function(){
	return $(".showTypeInput:checked").val();
};
DisneyScreen.RegionDetail.prototype.afterInsertRow=function(rowid,rowdata){
	var grid=this.grid;
	var tr=grid.find("tbody").find("tr")[rowid];
	var tds=$(tr).find("td");
...
DisneyScreen.RegionDetail.prototype.afterInsertRow2=function(rowid,rowdata){
...
DisneyScreen.RegionDetail.prototype.bindMenu=function(tr){
	var isHot=this.isHotspot();
...
DisneyScreen.RegionDetail.prototype.beforeMenu=function(e,context){
};
DisneyScreen.RegionDetail.prototype.menuHandler=function(context,e){
...
DisneyScreen.RegionDetail.prototype.showRecordAllKpis=function(targetName,rowId){
...
DisneyScreen.RegionDetail.prototype.getKpiDivTemplate=function(kpiName,value,unit,kpiId){
...
DisneyScreen.RegionDetail.prototype.drillCompareChart=function(td){
	var tdJQ=$(td);
...
DisneyScreen.RegionDetail.prototype.findCellInfo=function(cellName){
...
DisneyScreen.RegionDetail.prototype.drillSubHotspots=function(hotspot){
...
DisneyScreen.RegionDetail.prototype.drillSubCells=function(hotspot){
...
DisneyScreen.RegionDetail.prototype.locateCell=function(cellName){
	try{
...
DisneyScreen.RegionDetail.prototype.update=function (showLoadMask){
	if(showLoadMask){
//		this.showLoading();
	}
	this.clearGrid();
...
DisneyScreen.RegionDetail.prototype.subHotspotDataHandler=function (data){
	var hotspots=[];
...
DisneyScreen.RegionDetail.prototype.cellInfoHandler=function (data){
	this.cellList=data;
	var dm=LSMScreen.DataManager.getInstance();
	var cellType=this.getCellType();
	var laccis=[];
...
DisneyScreen.RegionDetail.prototype.streamCellDataHandler=function (data){
...
DisneyScreen.RegionDetail.prototype.wsCellDataHandler=function (data){
...
DisneyScreen.RegionDetail.prototype.streamDataHandler=function (data){
...
DisneyScreen.RegionDetail.prototype.wsDataHandler=function (data){
	this.wsData=data;
...
DisneyScreen.RegionDetail.prototype.resetGrid=function (){
	this.updateShowCol();
	this.loadCacheData();
};
DisneyScreen.RegionDetail.prototype.loadCacheData=function (page){
	if(page==null){
		page=1;
	} 
	var date0=new Date();
...
DisneyScreen.RegionDetail.prototype.updateRowClass=function(){
...
//对数据进行处理 换算 格式化等
DisneyScreen.RegionDetail.prototype.processRecordData=function(record,isCell,noConvert){
	var cols=this.getColList();
	var cols2=this.getColList2();
	var allCol=[];
	var i=0;
 ...
 * }
 */
DisneyScreen.RegionDetail.prototype.clearGrid=function(){
	if(this.grid){
...
DisneyScreen.RegionDetail.prototype.showColConfig = function(){
	var allCols=[];
	var selectedCols=[];
	var docWidth=1000;
	var docHeight=800;
...
DisneyScreen.AlarmList.TYPE_NEALARM="TYPE_NEALARM";
DisneyScreen.AlarmList.TYPE_PERFORMANCEALARM="TYPE_PERFORMANCEALARM";
DisneyScreen.AlarmList.TYPE_COMPLAIN="TYPE_COMPLAIN";


//网元告警数据
DisneyScreen.AlarmList.prototype.kpiType=LSMConsts.kpiTypeSig;
//网元告警数据
DisneyScreen.AlarmList.prototype.neAlarmList=[];
//性能告警数据
DisneyScreen.AlarmList.prototype.performanceAlarmList=[];
//投诉数据
DisneyScreen.AlarmList.prototype.complainList=[];

//jqgrid对象
DisneyScreen.AlarmList.prototype.grid;
//创建的table的dom对象
DisneyScreen.AlarmList.prototype.tableDom;
//表格呈现的内容的类型
DisneyScreen.AlarmList.prototype.tableType=DisneyScreen.AlarmList.TYPE_NEALARM;
//当前表格大小
//DisneyScreen.AlarmList.prototype.curSize="small";

DisneyScreen.AlarmList.prototype.pageSize=50;

DisneyScreen.AlarmList.prototype.pagerId="";
/** 
 * 创建组件内容
 * @public
 * @function 
 */
DisneyScreen.AlarmList.prototype.createComponent=function (){
	$(".alarmStatisticNum").on('click',this.switchAlarm.bind(this));
//	this.updateGridCol();
};

DisneyScreen.AlarmList.prototype.updateGridCol=function(){
	
	var cols=this.getCols();
	var drillFunc=null;
	
	switch(this.tableType){
		case DisneyScreen.AlarmList.TYPE_NEALARM:
			drillFunc=this.renderNeAlarm.bind(this);
			break;
		case DisneyScreen.AlarmList.TYPE_PERFORMANCEALARM:
			drillFunc=this.renderPerformanceAlarm.bind(this);
			break;
		case DisneyScreen.AlarmList.TYPE_COMPLAIN:
...
DisneyScreen.AlarmList.prototype.pagingData = function(btn){
...
DisneyScreen.AlarmList.prototype.showAlarmWin = function(title){
	var winWidth=1000;
	var winHeight=600;
	var docWidth=$(document).width();
	var docHeight=$(document).height();
	...
	$(win.content).css("overflow-x","auto");
...
DisneyScreen.AlarmList.prototype.getCols = function(){
	var cols=[];
	switch(this.tableType){
		case DisneyScreen.AlarmList.TYPE_NEALARM:
			cols=[
...
//			      	  {colName:'',name : 'isInArea',index : 'isInArea',width : 40},
			          ...
			          {colName:'迪士尼场馆',name : 'DISNEYLAND_SITE',index : 'DISNEYLAND_SITE',width : 260},
...
DisneyScreen.AlarmList.prototype.switchAlarm = function(evt){
	var type=$(evt.currentTarget).attr("name");
	this.switchTab(type);
};
DisneyScreen.AlarmList.prototype.filterClick = function(evt){
...
DisneyScreen.AlarmList.prototype.isFilter = function(){
	return $(".filterIcon").hasClass("filter12");
};
DisneyScreen.AlarmList.prototype.switchTab = function(type){
	if(type=="网元告警"){
		this.tableType=DisneyScreen.AlarmList.TYPE_NEALARM;
	}else if(type=="性能告警"){
		this.tableType=DisneyScreen.AlarmList.TYPE_PERFORMANCEALARM;
	}else if(type=="用户投诉"){
		this.tableType=DisneyScreen.AlarmList.TYPE_COMPLAIN;
	}
	this.showAlarmWin(type);
};
DisneyScreen.AlarmList.prototype.resetTab = function(){
...
DisneyScreen.AlarmList.prototype.renderNeAlarm=function(rowid,rowdata){
	var grid=this.grid;
	var tr=grid.find("tbody").find("tr")[rowid];
	$($(tr).find("td")[1]).addClass("alarmLv"+rowdata.ALARMSERVERITY+"Txt");
	$($(tr).find("td")[0]).text(rowdata.EVENTTIME.substring(5,19));
};
DisneyScreen.AlarmList.prototype.renderPerformanceAlarm=function(rowid,rowdata){
	var grid=this.grid;
...
DisneyScreen.AlarmList.prototype.locateArea=function(evt){
...
DisneyScreen.AlarmList.prototype.renderComplainRegion=function(rowid,rowdata){
	var grid=this.grid;
	var tr=grid.find("tbody").find("tr")[rowid];
...
DisneyScreen.AlarmList.prototype.isInArea=function(rowdata){
	var result=false;
...
DisneyScreen.AlarmList.prototype.update=function(showLoadMask){
	var dm=LSMScreen.DataManager.getInstance();
//	LSMConsts.area
...
DisneyScreen.AlarmList.prototype.neAlarmDataHandler=function(list){
	this.neAlarmList=[];
...
DisneyScreen.AlarmList.prototype.renderMapArea=function(list){
	try{
...
DisneyScreen.AlarmList.prototype.performanceAlarmDataHandler=function(list){
	var filterFlag=this.isFilter();
	if(filterFlag){
		var tmp=[];
		for(var i=0;i<list.length;i++){
...
DisneyScreen.AlarmList.prototype.complainDataHandler=function(list){
	var tmp=[];
	for(var i=0;i<list.length;i++){
		var record=list[i];
...
DisneyScreen.AlarmList.prototype.updateGridData=function(page){
	if(page==null){
		page=1;
	}
	this.resetTab();
...
DisneyScreen.AlarmList.prototype.updateRowClass=function(){
	this.grid.find("tr").removeClass("oddGrayTableRow");
	this.grid.find("tr:odd").addClass("oddGrayTableRow");
};
DisneyScreen.AlarmList.prototype.clearGrid=function(){
	this.grid.clearGridData();
};
