...
//NEI指标 选中列表
CIIE.Screen.prototype.netQualityKpis=[{
	key:'VOLTEeSRVCC成功率',
	name:'ESRVCC切换成功率',
	unit:'%',
	ratio:100,
...
CIIE.Screen.prototype.initialize=function(_hotspot){
	...
	$('#centerChartParent').on('dblclick',this.zoomChart.bind(this));
	
	$('.pyramid').on('click',this.drillTerminalModel.bind(this));
	...
	setInterval(this.refreshTime.bind(this),1000);
...
CIIE.Screen.prototype.initEcharts=function(ec){
	this.ec=echarts=ec;
	this.ec=echarts;
	this.ecConfig = echarts.config;
	this.chart00=echarts.init($("#chart0_0")[0],'marcarons');
	this.chart01=echarts.init($("#chart0_1")[0],'marcarons');
	this.chart02=echarts.init($("#chart0_2")[0],'marcarons');
	//this.chartPyramid=echarts.init($("#terminalrankChart")[0],'marcarons');
	this.terminalrankChart=echarts.init($("#terminalrankChart")[0],'marcarons');
	this.userChart=echarts.init($("#userChart")[0],'marcarons');
	this.roamRankChart=echarts.init($("#roamRankChart")[0],'marcarons');
...
CIIE.Screen.prototype.setNeiWsKpi=function(record,source){
	for(var key in record){
		...
		if($('#'+domId).length>0&&domSource==source){
			...
			var ratioTb=((normal-hb)/hb*100).toFixed(1);
...
CIIE.Screen.prototype.drillTerminalModel=function(e){
	if($('#terminalReturn').css('display')=='none'){
		var brand=$(e.currentTarget).attr('name');
...
//		var value=(barPercent*100).toFixed(2);
		...
		datas.push({value:showCnt, name:showName});
...
CIIE.Screen.prototype.drawNetQualityKpis=function(){
	var list=this.netQualityKpis;
	var html='';
	var count=0;
	var kpiNumCount=0;
	for(var i=0;i<list.length&&kpiNumCount<6;i++){
		var kpi=list[i];
		if(kpi.selected!=true) continue;
		if(kpiNumCount%3==0){
			html+='<div class="netQualityGroup" >';
		...
		html+='<div class="netQuality_ratiorow" style="line-height:20px;border:solid 0px #ff0000;margin-top:20px;padding:20px 15px 10px 35px;">'
			+'<div>同比</div><div class="icon-up"></div><div id="netQuality_tb_'+this.encodeDomId(kpi.key)+'">--%</div>'
			+'<div class="icon-splitline" style="height:50px;margin-left:10px;margin-top:-10px;margin-right:10px;"></div>'
			+'<div>环比</div><div class="icon-down"></div><div id="netQuality_hb_'+this.encodeDomId(kpi.key)+'">--%</div>'
			+'</div>';
		html+='</div>';
...
CIIE.Screen.prototype.drawCenterKpis=function(){
	...
	for(var i=0;i<hotspots.length;i++){
		if(hotspots[i].name!=this.hotspotInfo.hotCellInName){
			html+=this.getCenterKpiDiv(hotspots[i]);
		}
	}
	$('#centerKpi0').html(this.getCenterKpiDiv({icon:'shniec',name:this.hotspotInfo.hotCellInName},20));
	$('#centerKpi').html(html);
	//$(".mCustomScrollbar").mCustomScrollbar('update'); 
	$('.centerKpiTitle').on('click',this.centerHotClick.bind(this));
	//setInterval(this.scrollCenterKpis.bind(this),5000);
...
CIIE.Screen.prototype.updateMapHotspot=function(){
	...
	iframeWindow.updateMapHotspot(this.selectedHotspot);
};
...
CIIE.Screen.prototype.getCenterKpiDiv=function(hotspot,nomargin){
	...
	var style='';
	var spotname=hotspot.name;
	var icon=hotspot.icon;
	var prefix=spotname+'_center_';
	style='style="position:relative;height:220px;"';
	if(spotname.indexOf('酒店')!=-1){
		icon='hotel';
	}else if(spotname.indexOf('中心')!=-1){
		icon='center';
	}
	
	var div='<div id="'+this.encodeDomId(prefix)+'" class="centerHotspotKpiFrame" '+style+'>'
	...
	div+='<div class="icon-check" style="position:absolute;top:30px;left:30px;">'
		+'</div>';
...
CIIE.Screen.prototype.drawUserChart=function(result){
	var xArr=[];
	var userArr=[];
	var userArrLast=[];
	...
	for(var i=0;i<result.length;i++){
		...
		var hour=time.substring(11,16);
		xArr.push(hour);
		userArr.push(record["总用户数"]);
		userArrLast.push((record["总用户数"]/record["总用户数历史比"]).toFixed(0));
	}
